...
export async function loadAuditSession(id: string) {
  ...
  return {
    ...
    strawManRiskAudits: s.strawManRiskAudits.map((a) => ({
      ...
      targetIdentity: (a.targetIdentity ?? {}) as Record<string, unknown>,
      targetVersionResolved: a.targetVersionResolved,
      evidencePresence: (a.evidencePresence ?? {}) as Record<string, boolean>,
      verificationStatus: a.verificationStatus,
      critiqueSurvival: a.critiqueSurvival,
      comparativeSuperiorityUnverified: a.comparativeSuperiorityUnverified,
      dimensions: {
      ...
      groundRefs: (a.groundRefs as { segmentId?: string; charStart: number; charEnd: number; quote: string; role?: string }[]) ?? [],
      source: a.source,
    })),
